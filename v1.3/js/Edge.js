...
function Edge(model, config){
	...
	self.setDirection = function(value){
		...
		self.strength = value * self.attenuation;
	};

	self.setAttenuation = function(value){
		self.attenuation = value;
		self.strength = value * self.direction;
	}
	...
	self.addSignal = function(signal){
		...
		if(Edge.allSignals.length>model.MAX_SIGNALS){
			return;
		}

		// IF TOO MANY *ON THIS EDGE*, FORGET IT
		if(self.signals.length>model.MAX_SIGNALS_PER_EDGE){
			return;
		}
		...
		if(signal.age===undefined){
			...
			age = model.MAX_SIGNAL_AGE; // actually just make signals last "forever".
		}else{
			age = signal.age-1;
		}
		var newSignal = {
	...
	self.drawSignals = function(ctx){
		...
		for(var i=0; i<self.signals.length; i++){
			...
			ctx.scale(model.SIGNAL_SCALE_FACTOR, model.SIGNAL_SCALE_FACTOR);

			// Signal's COLOR, BLENDING
			var fromColor = model.COLOUR_NODE_LIST[self.from.hue];
			var toColor = model.COLOUR_NODE_LIST[self.to.hue];
			var blend;
	...
	self.update = function(speed){
		...
		else if(s>=-0.95) l="˗"; // EM dash, not hyphen.
		else l="–";
		self.label = l;
